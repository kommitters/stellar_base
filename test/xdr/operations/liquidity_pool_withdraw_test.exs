defmodule Stellar.XDR.Operations.LiquidityPoolWithdrawTest do
  use ExUnit.Case

  alias Stellar.XDR.{Int64, PoolID}
  alias Stellar.XDR.Operations.LiquidityPoolWithdraw
  ...
  describe "LiquidityPoolDeposit Operation" do
    setup do
      ...
      %{
        ...
        liquid_pool_withdraw:
          LiquidityPoolWithdraw.new(pool_id, amount, min_amount_a, min_amount_b),
        binary:
    ...
    test "encode_xdr/1", %{liquid_pool_withdraw: liquid_pool_withdraw, binary: binary} do
      {:ok, ^binary} = LiquidityPoolWithdraw.encode_xdr(liquid_pool_withdraw)
    end

    test "encode_xdr!/1", %{liquid_pool_withdraw: liquid_pool_withdraw, binary: binary} do
      ^binary = LiquidityPoolWithdraw.encode_xdr!(liquid_pool_withdraw)
    end

    test "decode_xdr/2", %{liquid_pool_withdraw: liquid_pool_withdraw, binary: binary} do
      {:ok, {^liquid_pool_withdraw, ""}} = LiquidityPoolWithdraw.decode_xdr(binary)
    end

    test "decode_xdr/2 with an invalid binary" do
      {:error, :not_binary} = LiquidityPoolWithdraw.decode_xdr(123)
    end

    test "decode_xdr!/2", %{liquid_pool_withdraw: liquid_pool_withdraw, binary: binary} do
      {^liquid_pool_withdraw, ^binary} = LiquidityPoolWithdraw.decode_xdr!(binary <> binary)
    end
  end
end
