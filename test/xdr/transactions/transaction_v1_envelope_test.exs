defmodule StellarBase.XDR.TransactionV1EnvelopeTest do
  ...
  describe "TransactionV1Envelope" do
    ...
    test "new/1", %{tx: tx, signatures: signatures} do
      %TransactionV1Envelope{tx: ^tx, signatures: ^signatures} =
        TransactionV1Envelope.new(tx, signatures)
    end

    test "encode_xdr/1", %{tx_envelope: tx_envelope, binary: binary} do
      {:ok, ^binary} = TransactionV1Envelope.encode_xdr(tx_envelope)
    end

    test "encode_xdr!/1", %{tx_envelope: tx_envelope, binary: binary} do
      ^binary = TransactionV1Envelope.encode_xdr!(tx_envelope)
    end

    test "decode_xdr/2", %{tx_envelope: tx_envelope, binary: binary} do
      {:ok, {^tx_envelope, ""}} = TransactionV1Envelope.decode_xdr(binary)
    end

    test "decode_xdr/2 with an invalid binary" do
      {:error, :not_binary} = TransactionV1Envelope.decode_xdr(123)
    end

    test "decode_xdr!/2", %{tx_envelope: tx_envelope, binary: binary} do
      {^tx_envelope, ^binary} = TransactionV1Envelope.decode_xdr!(binary <> binary)
    end
  end
