defmodule StellarBase.XDR.HostFunctionArgsTest do
  use ExUnit.Case
  ...
  describe "HostFunctionArgs" do
    setup do
      ...
      discriminants = [
        ...
        %{
          ...
          value: install_contract_code_args,
    ...
    test "new/1", %{discriminants: discriminants} do
      for %{host_function_type: host_function_type, value: host_function} <-
            discriminants do
        %HostFunctionArgs{value: ^host_function, type: ^host_function_type} =
          HostFunctionArgs.new(host_function, host_function_type)
      end
    end
    ...
    test "encode_xdr/1", %{discriminants: discriminants} do
      for %{
        ...
        xdr = HostFunctionArgs.new(host_function, host_function_type)
        {:ok, ^binary} = HostFunctionArgs.encode_xdr(xdr)
      end
    end
    ...
    test "encode_xdr!/1", %{discriminants: discriminants} do
      for %{
        ...
        xdr = HostFunctionArgs.new(host_function, host_function_type)
        ^binary = HostFunctionArgs.encode_xdr!(xdr)
      end
    end
    ...
    test "encode_xdr/1 with an invalid type", %{discriminants: [host_function | _rest]} do
      ...
      assert_raise XDR.EnumError,
                   ...
                   fn ->
                     host_function
                     |> HostFunctionArgs.new(host_function_type)
    ...
    test "decode_xdr/2", %{discriminants: discriminants} do
      for %{
        ...
        xdr = HostFunctionArgs.new(host_function, host_function_type)
        {:ok, {^xdr, ""}} = HostFunctionArgs.decode_xdr(binary)
      end
    end

    test "decode_xdr/2 with an invalid binary" do
      {:error, :not_binary} = HostFunctionArgs.decode_xdr(123)
    end
    ...
    test "decode_xdr!/2", %{discriminants: discriminants} do
      for %{
        ...
        xdr = HostFunctionArgs.new(host_function, host_function_type)
        {^xdr, ""} = HostFunctionArgs.decode_xdr!(binary)
      end
    end
  end
end
