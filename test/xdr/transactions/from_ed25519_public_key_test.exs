defmodule StellarBase.XDR.FromEd25519PublicKeyTest do
  use ExUnit.Case

  alias StellarBase.XDR.{Signature, UInt256, FromEd25519PublicKey}

  alias StellarBase.StrKey

  describe "FromEd25519PublicKey" do
    setup do
      key =
    ...
    test "new/2", %{key: key, signature: signature, salt: salt} do
      %FromEd25519PublicKey{key: ^key, signature: ^signature, salt: ^salt} =
        FromEd25519PublicKey.new(key, signature, salt)
    end

    test "encode_xdr/1", %{from_ed25519_public_key: from_ed25519_public_key, binary: binary} do
      {:ok, ^binary} = FromEd25519PublicKey.encode_xdr(from_ed25519_public_key)
    end
    ...
    test "encode_xdr!/1", %{
      ...
      ^binary = FromEd25519PublicKey.encode_xdr!(from_ed25519_public_key)
    end

    test "decode_xdr/2", %{from_ed25519_public_key: from_ed25519_public_key, binary: binary} do
      {:ok, {^from_ed25519_public_key, ""}} = FromEd25519PublicKey.decode_xdr(binary)
    end

    test "decode_xdr/2 with an invalid binary" do
      {:error, :not_binary} = FromEd25519PublicKey.decode_xdr(123)
    end
