defmodule StellarBase.XDR.OperationResultTrTest do
  use ExUnit.Case
  ...
  describe "CreateAccount OperationResultTr" do
    setup do
      ...
      %{
        op_type: op_type,
        value: value,
        op_inner_result: OperationResultTr.new(value, op_type),
    ...
    test "encode_xdr/1", %{op_inner_result: op_inner_result, binary: binary} do
      {:ok, ^binary} = OperationResultTr.encode_xdr(op_inner_result)
    end

    test "encode_xdr!/1", %{op_inner_result: op_inner_result, binary: binary} do
      ^binary = OperationResultTr.encode_xdr!(op_inner_result)
    end

    test "decode_xdr/2", %{op_inner_result: op_inner_result, binary: binary} do
      {:ok, {^op_inner_result, ""}} = OperationResultTr.decode_xdr(binary)
    end

    test "decode_xdr!/2", %{op_inner_result: op_inner_result, binary: binary} do
      {^op_inner_result, ^binary} = OperationResultTr.decode_xdr!(binary <> binary)
    end
