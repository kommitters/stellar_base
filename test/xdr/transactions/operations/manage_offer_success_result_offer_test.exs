defmodule StellarBase.XDR.ManageOfferSuccessResultOfferTest do
  use ExUnit.Case

  import StellarBase.Test.Utils

  alias StellarBase.XDR.{OfferEntryExt, Int32, Int64, OfferEntry, Price, Uint32, Void}
  alias StellarBase.XDR.{ManageOfferSuccessResultOffer, ManageOfferEffect}

  describe "ManageOfferSuccessResultOffer" do
    setup do
      ...
      type = ManageOfferEffect.new(:MANAGE_OFFER_CREATED)

      value = OfferEntry.new(seller_id, offer_id, selling, buying, amount, price, flags, ext)

      %{
        type: type,
        value: value,
        manage_offer: ManageOfferSuccessResultOffer.new(value, type),
        binary:
    ...
    test "new/1", %{value: value, type: type} do
      %ManageOfferSuccessResultOffer{value: ^value, type: ^type} =
        ManageOfferSuccessResultOffer.new(value, type)
    end

    test "encode_xdr/1", %{manage_offer: manage_offer, binary: binary} do
      {:ok, ^binary} = ManageOfferSuccessResultOffer.encode_xdr(manage_offer)
    end

    test "encode_xdr!/1", %{manage_offer: manage_offer, binary: binary} do
      ^binary = ManageOfferSuccessResultOffer.encode_xdr!(manage_offer)
    end

    test "decode_xdr/2", %{manage_offer: manage_offer, binary: binary} do
      {:ok, {^manage_offer, ""}} = ManageOfferSuccessResultOffer.decode_xdr(binary)
    end

    test "decode_xdr!/2", %{manage_offer: manage_offer, binary: binary} do
      {^manage_offer, ^binary} = ManageOfferSuccessResultOffer.decode_xdr!(binary <> binary)
    end

    test "decode_xdr/2 with an invalid binary" do
      {:error, :not_binary} = ManageOfferSuccessResultOffer.decode_xdr(123)
    end
  end
end
