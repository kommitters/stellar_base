defmodule StellarBase.XDR.Operations.FeeBumpTransactionTest do
  ...
  describe "FeeBumpInnerTx" do
    setup do
      ...
      inner_tx = build_fee_bump_inner_tx()
      ext = Ext.new()
      ...
      %{
        ...
        inner_tx: inner_tx,
        ext: ext,
        fee_bump_tx: FeeBumpTransaction.new(fee_source, fee, inner_tx, ext),
        binary:
          <<0, 0, 0, 0, 155, 142, 186, 248, 150, 56, 85, 29, 207, 158, 164, 247, 67, 32, 113, 16,
            ...
            2, 0, 0, 0, 1, 0, 0, 0, 0, 108, 108, 200, 144, 232, 101, 96, 6, 227, 152, 210, 30,
            ...
            219, 0, 0, 0, 0, 59, 154, 202, 0, 0, 0, 0, 1, 0, 0, 0, 0, 155, 142, 186, 248, 150, 56,
            85, 29, 207, 158, 164, 247, 67, 32, 113, 16, 107, 135, 171, 14, 45, 179, 214, 155,
            117, 165, 56, 34, 114, 247, 89, 216, 0, 0, 0, 0, 0, 0, 0, 2, 84, 82, 87, 73, 0, 0, 0,
            56, 83, 65, 80, 86, 86, 85, 81, 50, 71, 55, 53, 53, 75, 71, 81, 79, 79, 89, 53, 65,
            51, 65, 71, 84, 77, 87, 67, 67, 81, 81, 84, 74, 77, 71, 83, 88, 65, 85, 75, 77, 70,
            84, 52, 53, 79, 70, 67, 76, 55, 78, 67, 83, 84, 82, 87, 73, 80, 76, 85, 89, 0, 0, 0,
            56, 83, 66, 86, 78, 81, 76, 73, 68, 83, 55, 86, 51, 78, 65, 79, 89, 84, 65, 84, 84,
            50, 54, 81, 76, 55, 89, 52, 83, 54, 67, 50, 88, 52, 89, 78, 55, 80, 78, 53, 70, 73,
            74, 54, 74, 85, 65, 78, 52, 85, 86, 52, 89, 80, 76, 85, 89, 0, 0, 0, 0>>
      }
    end

    test "new/1", %{fee_source: fee_source, fee: fee, inner_tx: inner_tx, ext: ext} do
      %FeeBumpTransaction{fee_source: ^fee_source, inner_tx: ^inner_tx} =
        FeeBumpTransaction.new(fee_source, fee, inner_tx, ext)
    end

    test "encode_xdr/1", %{fee_bump_tx: fee_bump_tx, binary: binary} do
      {:ok, ^binary} = FeeBumpTransaction.encode_xdr(fee_bump_tx)
    end

    test "encode_xdr!/1", %{fee_bump_tx: fee_bump_tx, binary: binary} do
      ^binary = FeeBumpTransaction.encode_xdr!(fee_bump_tx)
    end

    test "decode_xdr/2", %{fee_bump_tx: fee_bump_tx, binary: binary} do
      {:ok, {^fee_bump_tx, ""}} = FeeBumpTransaction.decode_xdr(binary)
    end

    test "decode_xdr!/2", %{fee_bump_tx: fee_bump_tx, binary: binary} do
      {^fee_bump_tx, ^binary} = FeeBumpTransaction.decode_xdr!(binary <> binary)
    end

    test "decode_xdr/2 with an invalid binary" do
      {:error, :not_binary} = FeeBumpTransaction.decode_xdr(123)
    end
  end
