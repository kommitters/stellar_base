defmodule StellarBase.XDR.HashIDPreimageContractIDTest do
  ...
  alias StellarBase.XDR.{HashIDPreimageContractID, Hash, Uint256}
  ...
  describe "HashIDPreimageContractID" do
    setup do
      ...
      %{
        ...
        salt: salt,
        contract_id_contract_id: HashIDPreimageContractID.new(network_id, contract_id, salt),
        binary:
    ...
    test "new/1", %{
      ...
      %HashIDPreimageContractID{
        ...
        contract_id: ^contract_id,
        salt: ^salt
      } = HashIDPreimageContractID.new(network_id, contract_id, salt)
    end

    test "encode_xdr/1", %{contract_id_contract_id: contract_id_contract_id, binary: binary} do
      {:ok, ^binary} = HashIDPreimageContractID.encode_xdr(contract_id_contract_id)
    end

    test "encode_xdr!/1", %{contract_id_contract_id: contract_id_contract_id, binary: binary} do
      ^binary = HashIDPreimageContractID.encode_xdr!(contract_id_contract_id)
    end

    test "decode_xdr/2", %{contract_id_contract_id: contract_id_contract_id, binary: binary} do
      {:ok, {^contract_id_contract_id, ""}} = HashIDPreimageContractID.decode_xdr(binary)
    end
    ...
    test "decode_xdr!/2", %{contract_id_contract_id: contract_id_contract_id, binary: binary} do
      {^contract_id_contract_id, ""} = HashIDPreimageContractID.decode_xdr!(binary)
    end
  end
end
