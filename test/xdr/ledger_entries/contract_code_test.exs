defmodule StellarBase.XDR.LedgerKeyContractCodeTest do
  ...
  alias StellarBase.XDR.{LedgerKeyContractCode, Hash}
  ...
  describe "LedgerKeyContractCode" do
    setup do
      ...
      %{
        hash: hash,
        contract_code_entry: LedgerKeyContractCode.new(hash),
        binary: "GCIZ3GSM5XL7OUS4UP64THMDZ7CZ3ZWN"
      }
    end

    test "new/1", %{hash: hash} do
      %LedgerKeyContractCode{hash: ^hash} = LedgerKeyContractCode.new(hash)
    end

    test "encode_xdr/1", %{contract_code_entry: contract_code_entry, binary: binary} do
      {:ok, ^binary} = LedgerKeyContractCode.encode_xdr(contract_code_entry)
    end
    ...
    test "encode_xdr!/1", %{
      ...
      ^binary = LedgerKeyContractCode.encode_xdr!(contract_code_entry)
    end

    test "decode_xdr/2", %{contract_code_entry: contract_code_entry, binary: binary} do
      {:ok, {^contract_code_entry, ""}} = LedgerKeyContractCode.decode_xdr(binary)
    end
    ...
    test "decode_xdr!/2", %{
      ...
      {^contract_code_entry, ^binary} = LedgerKeyContractCode.decode_xdr!(binary <> binary)
    end
  end
end
