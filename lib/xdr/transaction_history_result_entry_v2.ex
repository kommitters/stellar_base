defmodule StellarBase.XDR.TransactionHistoryResultEntryV2 do
  @moduledoc """
  Automatically generated by xdrgen
  DO NOT EDIT or your changes may be overwritten

  Target implementation: elixir_xdr at https://hex.pm/packages/elixir_xdr

  Representation of Stellar `TransactionHistoryResultEntryV2` type.
  """

  @behaviour XDR.Declaration

  alias StellarBase.XDR.{
    Uint32,
    TransactionResultSetV2,
    TransactionHistoryResultEntryV2Ext
  }

  @struct_spec XDR.Struct.new(
    ledger_seq: Uint32,
    tx_result_set: TransactionResultSetV2,
    ext: TransactionHistoryResultEntryV2Ext
  )

  @type type_ledger_seq :: Uint32.t()
  @type type_tx_result_set :: TransactionResultSetV2.t()
  @type type_ext :: TransactionHistoryResultEntryV2Ext.t()

  @type t :: %__MODULE__{ledger_seq: type_ledger_seq(), tx_result_set: type_tx_result_set(), ext: type_ext()}

  defstruct [:ledger_seq, :tx_result_set, :ext]

  @spec new(ledger_seq :: type_ledger_seq(), tx_result_set :: type_tx_result_set(), ext :: type_ext()) :: t()
  def new(
    %Uint32{} = ledger_seq,
    %TransactionResultSetV2{} = tx_result_set,
    %TransactionHistoryResultEntryV2Ext{} = ext
  ),
  do: %__MODULE__{ledger_seq: ledger_seq, tx_result_set: tx_result_set, ext: ext}

  @impl true
  def encode_xdr(%__MODULE__{ledger_seq: ledger_seq, tx_result_set: tx_result_set, ext: ext}) do
    [ledger_seq: ledger_seq, tx_result_set: tx_result_set, ext: ext]
    |> XDR.Struct.new()
    |> XDR.Struct.encode_xdr()
  end

  @impl true
  def encode_xdr!(%__MODULE__{ledger_seq: ledger_seq, tx_result_set: tx_result_set, ext: ext}) do
    [ledger_seq: ledger_seq, tx_result_set: tx_result_set, ext: ext]
    |> XDR.Struct.new()
    |> XDR.Struct.encode_xdr!()
  end

  @impl true
  def decode_xdr(bytes, struct \\ @struct_spec)

  def decode_xdr(bytes, struct) do
    case XDR.Struct.decode_xdr(bytes, struct) do
      {:ok, {%XDR.Struct{components: [ledger_seq: ledger_seq, tx_result_set: tx_result_set, ext: ext]}, rest}} ->
        {:ok, {new(ledger_seq, tx_result_set, ext), rest}}
      error -> error
    end
  end

  @impl true
  def decode_xdr!(bytes, struct \\ @struct_spec)

  def decode_xdr!(bytes, struct) do
    {%XDR.Struct{components: [ledger_seq: ledger_seq, tx_result_set: tx_result_set, ext: ext]}, rest} =
      XDR.Struct.decode_xdr!(bytes, struct)
    {new(ledger_seq, tx_result_set, ext), rest}
  end
end
